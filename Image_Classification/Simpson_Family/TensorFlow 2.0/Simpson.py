...
trainImage = tf.data.TFRecordDataset("simpsonTrainImage.tfrec").map(preprocessImage)
trainLabel = tf.data.TFRecordDataset("simpsonTrainLabel.tfrec").map(_parse_function)

validImage = tf.data.TFRecordDataset("simpsonValidImage.tfrec").map(preprocessImage)
validLabel = tf.data.TFRecordDataset("simpsonValidLabel.tfrec").map(_parse_function)


trainDataset = tf.data.Dataset.zip((trainImage,trainLabel))
trainDataset = trainDataset.shuffle(buffer_size=16215)
...
validAccuracy = tf.keras.metrics.SparseCategoricalAccuracy(name='valid_accuracy')
